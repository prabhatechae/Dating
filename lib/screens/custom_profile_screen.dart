...
// Variables
  ...
  void initState() {
    ...
    initUserSettings();
    videocontroller = VideoPlayerController.network(widget.user.userVideoUrl);
    // 'https://firebasestorage.googleapis.com/v0/b/datingapp-f7b21.appspot.com/o/videos%2F2023-10-13%2011%3A58%3A10.191770.mp4?alt=media&token=8d740b90-00fd-4d3d-9758-fc27559e0975');
    videocontroller!.initialize().then((_) {
      setState(() {});
    }).catchError((error) {
      // Handle the error and set videocontroller to null
    ...
    FirebaseFirestore.instance
      ...
      if (doc.exists) {
        setState(() {
          dropDownValue = doc["user_dropdown_list"];
        });
      }
    });
  }
  ...
  Future<void> pickVideoFromGallery() async {
    ...
    if (videopicked != null) {
      ...
      // Upload the video to Firebase Storage and get the download URL
      String? downloadUrl = await uploadVideoToFirebaseStorage(video);
      if (downloadUrl != null) {
        await firestore.collection("Users").doc(widget.user.userId).update({
          "user_video_link": downloadUrl,
        });
  ...
  Future<String?> uploadVideoToFirebaseStorage(File? videoFile) async {
    try {
      ...
      return downloadUrl; // Returns the URL of the uploaded video
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    _phoneController.text = widget.user.userPhoneNumber; //+911234567890
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                            ...
                            children: [
                              ...
                              SizedBox(
                                ...
                                child: TextFormField(
                                  style: const TextStyle(
                                      color: Colors
                                          .white), // Text color in black theme
                                  cursorColor: Colors
                                      .white, // Cursor color in black theme
                                  decoration: InputDecoration(
                                    contentPadding: const EdgeInsets.all(0),
                                    filled:
                                        true, // Fill the background with black
                                    fillColor: Colors
                                        .black, // Background color in black theme
                                ...
                                child: TextFormField(
                                  ...
                                  decoration: InputDecoration(
                                    ...
                                    fillColor: Colors
                                        .black, // Background color in black theme
                                    hintText:
                                        'Write your bio here', // Placeholder text
                                    hintStyle: const TextStyle(
                                        color: Colors
                                            .white), // Placeholder color in black theme
                                    border: OutlineInputBorder(
                                      borderRadius: BorderRadius.circular(10),
                                      borderSide: BorderSide.none,
                                    ),
                        ...
                        Container(
                          ...
                          child: DropdownButtonHideUnderline(
                            ...
                            onChanged: (value) async {
                              ...
                              // Update the document with the new dropdown value directly in Firebase
                              await FirebaseFirestore.instance
                                  .collection("Users")
                                  .doc(widget.user.userId)
                                  .update({
                                "user_dropdown_list": dropDownValue
                              }).then((_) {
                                // Update successful
                                print(
                                    "Value updated successfully in Firebase.");
                              }).catchError((error) {
                                // Handle error
                                print(
                                    "Error updating value in Firebase: $error");
                              });
                            },
                        ...
                        Card(
                          color: Colors
                              .black, // Background color of the Card in black theme
                          child: ListTile(
                            leading: const Icon(
                              Icons.favorite_border_outlined,
                              color: Colors
                                  .white, // Icon color in white for black theme
                              size: 30,
                            ),
                            title: const Text(
                              'INTERESTS',
                              style: TextStyle(
                                  fontSize: 18,
                                  color: Colors
                                      .white), // Text color in white for black theme
                            ),
                            onTap: () {
                              Navigator.push<void>(
                                context,
                          ...
                          child: ListTile(
                            leading: _hideProfile
                                ? const Icon(Icons.visibility_off,
                                    color: Colors.white,
                                    size:
                                        30) // Icon color in white for black theme
                            ...
                            trailing: Switch(
                              activeColor: Colors
                                  .white, // Switch active color in white for black theme
      ...
      return FutureBuilder(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.done) {
            ...
            // While the video is initializing, show a circular progress indicator
            // return CircularProgressIndicator();
          }
        },
      );
    } else {
      // If no video is available, show the play button
      return CircleAvatar(
        radius: 18,
        backgroundColor: Theme.of(context).primaryColor,
        child: GestureDetector(
  ...
  void _selectVideo({required String imageUrl, required String path}) async {
    await showModalBottomSheet(
        context: context,
        backgroundColor: Colors.transparent,
        builder: (context) => ImageSourceSheet(
              onImageSelected: (image) async {
                if (image != null) {
                  ...
                  await UserModel().updateProfileVideo(
                      videoFile: image, oldVideoUrl: imageUrl, path: 'videos');
                  // Hide dialog
                  _pr.hide();
                  // close modal
                  Navigator.of(context).pop();
