...
class UserModel extends Model {
  ...
  Future<void> updateProfileVideo(
    ...
    if (path == 'videos') {
      uploadPath = 'uploads/users/videos/videos';
    } else {
      uploadPath = 'uploads/users/videos/gallery';
    }
    ...
    final videoLink = await uploadFile(
      file: videoFile,
      path: uploadPath,
      userId: user.userId,
    );
    ...
    if (path == 'videos') {
      await updateUserData(userId: user.userId, data: {USER_VIDEO_LINK: videoLink});
    } else {
      await updateUserData(
        userId: user.userId,
        data: {'$USER_GALLERY.video_$index': videoLink},
      );
    }
  }
